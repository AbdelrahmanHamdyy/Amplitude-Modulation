...
% Adjust all lengths of the 3 input signals to be the same (having max length)
...
demodulate(finalModulatedSignal, carrier_t1, fs, fp, "Sync_1");
demodulate(finalModulatedSignal, carrier_t2, fs, fp, "Sync_2");
demodulate(finalModulatedSignal, carrier_t3, fs, fp, "Sync_3");

% Phase Shift = 10
[carrier1_10, carrier2_10, carrier3_10] = generateCarriersWithPhase(fc_1, fc_2, time, 10);
demodulate(finalModulatedSignal, carrier1_10, fs, fp, "PhaseShift10_1");
demodulate(finalModulatedSignal, carrier2_10, fs, fp, "PhaseShift10_2");
demodulate(finalModulatedSignal, carrier3_10, fs, fp, "PhaseShift10_3");

% Phase Shift = 30
[carrier1_30, carrier2_30, carrier3_30] = generateCarriersWithPhase(fc_1, fc_2, time, 30);
demodulate(finalModulatedSignal, carrier1_30, fs, fp, "PhaseShift30_1");
demodulate(finalModulatedSignal, carrier2_30, fs, fp, "PhaseShift30_2");
demodulate(finalModulatedSignal, carrier3_30, fs, fp, "PhaseShift30_3");

% Phase Shift = 90
[carrier1_90, carrier2_90, carrier3_90] = generateCarriersWithPhase(fc_1, fc_2, time, 90);
demodulate(finalModulatedSignal, carrier1_90, fs, fp, "PhaseShift90_1");
demodulate(finalModulatedSignal, carrier2_90, fs, fp, "PhaseShift90_2");
demodulate(finalModulatedSignal, carrier3_90, fs, fp, "PhaseShift90_3");
...
demodulate(finalModulatedSignal, carrier1_2Hz, fs, fp, "Fc2Hz_1");
demodulate(finalModulatedSignal, carrier2_2Hz, fs, fp, "Fc2Hz_2");
demodulate(finalModulatedSignal, carrier3_2Hz, fs, fp, "Fc2Hz_3");
...
demodulate(finalModulatedSignal, carrier1_10Hz, fs, fp, "Fc10Hz_1");
demodulate(finalModulatedSignal, carrier2_10Hz, fs, fp, "Fc10Hz_2");
demodulate(finalModulatedSignal, carrier3_10Hz, fs, fp, "Fc10Hz_3");

function draw(time, freq, i, mt, mf, phase, T)
    % Plot Message in Time Domain
    figure(i)
    subplot(3, 1, 1)
    plot(time, mt)
    xlabel('Time')
    ...
    % Plot Message Amplitude in Frequency Domain
    subplot(3, 1, 2)
    plot(freq, mf)
    xlabel('Freq')
    ylabel('Amp')
    ...
    % Plot Message Phase in Frequency Domain
    subplot(3, 1, 3)
    plot(freq, phase)
    xlabel('Freq')
    ylabel('Phase')
...
function demodulate(signal, carrier, fs, fp, filename)
    % Re-multiply by the carrier to restore the signal with half magnitude
    % So we double the output to get the exact original signal
    ...
    % Output the final result to an audio file
...
function [carrier1, carrier2, carrier3] = generateCarriersWithPhase(fc_1, fc_2, time, deg)
    ...
    % Get carrier for each signal with an additional phase shift
...
function [carrier1, carrier2, carrier3] = generateCarriersWithDifferentFc(fc_1, fc_2, time, dfc)
    % Get new omega with a constant added to the carrier frequency
    wc_1 = 2*pi * (fc_1 + dfc);
    wc_2 = 2*pi * (fc_2 + dfc);
    
    % Get new carriers just like we did in synchronous demodulation
